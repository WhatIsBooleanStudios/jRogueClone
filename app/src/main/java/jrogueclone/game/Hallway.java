...
public class Hallway {
    public Hallway(Vector2D startPosition, Vector2D endPosition, char[][] roomMap) {
        ...
        } else if (startPosition.getY() < Global.rows - 1
                && roomMap[startPosition.getX()][startPosition.getY() + 1] == ' ') {
            m_Waypoints.add(new Vector2D(startPosition.getX(), startPosition.getY() + 1));
        ...
        } else if (startPosition.getX() < Global.columns - 1
                && roomMap[startPosition.getX() + 1][startPosition.getY()] == ' ') {
            m_Waypoints.add(new Vector2D(startPosition.getX() + 1, startPosition.getY()));
        }
        ...
        } else if (endPosition.getX() < Global.columns - 1
                && roomMap[endPosition.getX() + 1][endPosition.getY()] == ' ') {
            finalWaypoint = (new Vector2D(endPosition.getX() + 1, endPosition.getY()));
        }
        ...
        while (cursorPos.getX() != finalWaypoint.getX() || cursorPos.getY() != finalWaypoint.getY()) {
            // Move until the x value is equal to that of the waypoint or until we hit a
            // wall
            boolean movedX = false;
            while (!((cursorPos.getX() == finalWaypoint.getX()) ||
            ...
            // Handle the case where our x value equals the end point except our y value
            // does not and
            // There is a wall in our way from going up or down. This means that we need to
            // deviate from the
            // target x value until we can move up or down. Afterwards, we recalculate the x
            // and y direction
            // that we need to move
            if (!cursorPos.equals(finalWaypoint)) {
                boolean adjustX = false;
                while ((cursorPos.getY() != Global.rows - 1 && yDirection == DOWN
                        && roomMap[cursorPos.getX()][cursorPos.getY() + 1] == '#') ||
                        (cursorPos.getY() > 0 && yDirection == UP
                                && roomMap[cursorPos.getX()][cursorPos.getY() - 1] == '#')) {
                    cursorPos.setX(cursorPos.getX() + (xDirection == LEFT ? -1 : 1));
                    adjustX = true;
                }
            ...
            while (!((yDirection == DOWN && cursorPos.getY() == Global.rows - 1) ||
            ...
            // Handle the case where our y value equals the end point except our x value
            // does not and
            // There is a wall in our way from going left or right. This means that we need
            // to deviate from the
            // target y value until we can move right or left. Afterwards, we recalculate
            ...
            if (!cursorPos.equals(finalWaypoint)) {
                ...
                while ((roomMap[cursorPos.getX() + 1][cursorPos.getY()] == '#') ||
    ...
    public boolean contains(Vector2D position) {
        for (Vector2D hallwayPoint : this.m_Waypoints) {
            if (hallwayPoint.equals(position))
                return true;
        }
        return false;
    }
